...
**Agent observability** (`pnpm dev:svg`) — renders the SVG-native Web Components from `@kibadist/agentui-svg` from a **real, instrumented agent run** (no canned data, **no offline mock — requires `ANTHROPIC_API_KEY`**).
- `examples/svg-api/` (`:3003`) — an SRE incident agent. SQLite seeds services/deploys/error_logs/metrics. The agent runs a real `generateText` loop with read tools (`list_services`, `get_deploys`, `query_error_logs`, `get_metrics`) + a write tool `propose_rollback`. Each tool's `execute` is **wrapped** so a per-turn `RunRecorder` (`src/agent/run-recorder.ts`) captures real tool name/args/duration/result and streams the components live: first `ui.append`, then `ui.replace` (key + `replace:true`) on each step. `propose_rollback` emits a `review-checkpoint` built from real metrics/logs and sets the state machine to "awaiting". Without a key it emits a "set the key" message. The `RunRecorder` is framework-pure so `test/instrumentation.test.ts` drives it without a model (run via its own `vitest.config.ts` — NOT in the root suite).
- `examples/svg-app/` (`:3002`) — registry of thin React wrappers around the custom elements (`workflow-canvas`, `tool-timeline`, `review-checkpoint`, `memory-map`, `state-machine`, `text-block`) + quick-action investigation prompts. The SVG package is **registered client-side only** (its classes extend `HTMLElement`, undefined during SSR — see `components/svg-element.tsx`, dynamic `import("@kibadist/agentui-svg/register")` in an effect). **`SvgElement` applies config like `layout`/`density` via `setAttribute`, never as React props** — React 19 would set them as element properties and `layout` is a getter-only accessor (throws). The page's `sender` stamps the live `sessionId` onto every action (component-emitted actions start with an empty one). Selecting a node/item/state sends `agent.inspect`; checkpoint decisions send `agent.decision`.

The clinic pair's allowed component types (`COMPONENT_DEFS` in `agent.service.ts`) and its frontend registry schemas (`components/schemas.ts`) mirror each other — keep them in sync when changing component props. (The svg pair emits component nodes directly from the `RunRecorder`, so the shapes there must match `svg-app/components/schemas.ts`.)
